...
- Implemented PR feedback fixes (callback non-blocking, PDA validation, interface cleanup, relayer timeouts, ECDSA low-s)
- Added deploy script and transferOwnership, expanded Foundry tests
- Added Solana program tests and request size checks
- Consolidated Solana relayer parsing helpers and added relayer .gitignore
- Updated example config URL and clarified enum mappings
- Re-vendored Solana dependencies for toolchain compatibility
- Verified `cargo build-sbf` for Solana gateway + consumer and `cargo test` for Solana program tests
