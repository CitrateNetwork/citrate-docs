...
use crate::parsing::combinator::{ascii_char, sign, Sign};
...
impl<const CONFIG: EncodedConfig> Iso8601<CONFIG> {
        ...
        move |input| {
            ...
            *extended_kind = match ascii_char::<b'-'>(input) {
            ...
            let mut ret_error = match parsed_month_day {
                Ok(ParsedItem(input, (month, day))) => {
                    *parsed = try_likely_ok!(try_likely_ok!(try_likely_ok!(parsed
                        .with_year(year)
            ...
            if let Some(ParsedItem(input, ordinal)) = dayo(input) {
                *parsed = try_likely_ok!(try_likely_ok!(parsed
                    .with_year(year)
                    .ok_or(InvalidComponent("year")))
        ...
        move |mut input| {
            ...
            match hour {
                ...
                (hour, Some(fractional_part)) => {
                    *parsed =
                        try_likely_ok!(try_likely_ok!(try_likely_ok!(try_likely_ok!(parsed
                            .with_hour_24(hour)
                    ...
                    *parsed = try_likely_ok!(try_likely_ok!(try_likely_ok!(parsed
                    ...
                    .with_subsecond(
                        (fractional_part * Nanosecond::per_t::<f64>(Minute)
                            % Nanosecond::per_t::<f64>(Second)) as u32,
                    )
                    .ok_or(InvalidComponent("subsecond")));
                    return Ok(input);
                }
    ...
    pub(crate) fn parse_offset<'a>(
        ...
        move |input| {
            ...
            let mut input = try_likely_ok!(hour(input)
