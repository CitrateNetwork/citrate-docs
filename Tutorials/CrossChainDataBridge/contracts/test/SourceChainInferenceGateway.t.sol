...
contract SourceChainInferenceGatewayTest is Test {
    ...
    function testRejectDuplicateSigner() public {
        ...
        signatures[0] = _signAttestation(relayerKey1, a);
        signatures[1] = _signAttestation(relayerKey1, a);
    ...
    function testMarkFailedTyped() public {
        bytes32 modelHash = keccak256("model");
        bytes memory inputData = abi.encodePacked("hello");
        uint256 deadline = block.timestamp + 100;
        ...
        SourceChainInferenceGateway.Attestation memory a =
            _buildAttestation(requestId, modelHash, inputData, bytes32(0));
        ...
        gateway.markFailedTyped(requestId, a.citrateRequestId, attestations, signatures);

        (, , , , , , , , SourceChainInferenceGateway.RequestStatus status, , , ) =
            gateway.getRequest(requestId);
        assertEq(uint256(status), uint256(SourceChainInferenceGateway.RequestStatus.Failed));
    }

    function testZeroQuorumRejected() public {
        vm.expectRevert(bytes("Quorum must be > 0"));
        new SourceChainInferenceGateway(40204, 0);
    }
        ...
        returns (bytes memory)
    {
        ...
        (v, r, s) = _normalizeSignature(v, r, s);
        return abi.encodePacked(r, s, v);
    }

    function _normalizeSignature(uint8 v, bytes32 r, bytes32 s)
        internal
        pure
