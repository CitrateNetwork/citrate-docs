...
async function handleRequest(requestId) {
  ...
  try {
    ...
    // Citrate InferenceRouter: 1=Processing, 2=Completed, 3=Failed.
    // Source gateway: 0=Pending, 1=Completed, 2=Failed.
    while (true) {
      const pollNow = Math.floor(Date.now() / 1000);
      if (pollNow >= deadline) {
        console.warn(`Deadline reached for request ${requestId}; stopping Citrate poll.`);
        return;
      }
    ...
    if (confirmationDepth > 0) {
      while ((await citrateProvider.getBlockNumber()) - blockNumber < confirmationDepth) {
        const confirmNow = Math.floor(Date.now() / 1000);
        if (confirmNow >= deadline) {
          console.warn(`Deadline reached for request ${requestId}; skipping confirmations.`);
          return;
        }
        await sleep(config.pollIntervalMs || 2000);
      }
