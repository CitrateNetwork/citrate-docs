...
Callback failures should not revert finalization; treat the callback as best-effort and emit an event if it fails.
...
7. If `callbackTarget` and `callbackSelector` are set, attempt the callback with `(requestId, outputData)` without reverting on failure
