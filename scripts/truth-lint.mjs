...
const FINALITY_CURRENT = [
  ...
  /\breorg(ani[sz]ation)?s? (are|is|become) (impossible|not possible)/i,
  /\bfinality is guaranteed|\bguaranteed finality/i,
  /\b(settlement|a block|blocks?|transactions?) (is|are|becomes?) final once/i,
  /\bcan(no|')?t be (rolled back|reverted)|\bcannot be (rolled back|reverted)/i,
];
...
const ALWAYS = [
  ...
  [/(npm (i|install)|pnpm add|yarn add|bun add|npx)\s+citrate-js\b|from ['"]citrate-js['"]/i, "citrate-js is a stale personal-account package; use @citratelabs/sdk"],
  ...
  [/\b(retained evidence|every decryption|all decryptions?|identity data|DSAR|any decrypt\w*)\b.{0,80}\b(dual[\s-]?control|two (distinct )?admin(istrator)?s|both admins)\b|\b(dual[\s-]?control|two (distinct )?admin(istrator)?s|both admins)\b.{0,80}\b(retained evidence|every decryption|all decryptions?|identity data|DSAR)\b/i, "do not claim dual control over identity data or every decryption"],
  [/every (node )?(operator|machine|node|person|validator|producer)\b.{0,80}\b(identity|KYC)[\s-]verified|\bidentity[\s-]verified through VERI\b|\b(operators|validators) are (identity|KYC)[\s-]verified|\bonly (identity|KYC)[\s-]verified (operators|nodes|validators)/i, "node and consensus code do not check operator identity"],
  ...
  [/\bpaid (gateway|inference|routes|calls|rails)\b.{0,40}\b(is|are) (now )?(live|generally available)/i, "paid rails are not deployed"],
  [/\bseventy[\s-]six (contracts|deployed)/i, "57 of 76 book entries have code; say so"],
